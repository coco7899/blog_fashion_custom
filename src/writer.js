...
12. 본문의 마지막 문단에는 반드시 "이 글에 제휴하면 좋은 제품 후보: **제품명**"을 한 번 넣고, 왜 이 제품이 생활 실천에 도움이 되는지 한 문장으로 설명하세요. 바로 앞 본문과는 빈 줄 두 줄을 두어 분리되게 쓰세요. 이는 사용자가 실제 제휴 여부를 판단할 참고 후보이며, 자동 링크가 아닙니다. 링크·가격·할인·구매 후기·치료 효과는 쓰지 마세요. 측정 도구를 추천할 때는 진단·처방을 대신하지 않는다는 점을 함께 밝히세요. 해시태그 6개는 이 문단 다음, 글 전체의 맨 마지막에만 배치합니다.
...
function getSafeProductCandidate(topic = {}) {
  ...
  if (/요로|결석|수분|탈수/.test(subject)) return '눈금 있는 휴대용 물병';
  if (/혈압|심장|맥박/.test(subject)) return '건강 기록 노트';
  if (/질염|질\s*건강|분비물|비뇨/.test(subject)) return '건강 기록 노트';
  if (/수면|불면/.test(subject)) return '수면 기록 노트';
  return '생활 기록 노트';
}

function makeRecommendationParagraph(topic) {
  const product = getSafeProductCandidate(topic);
  return `반복되는 불편이 있다면 오늘부터 증상이 나타난 시간과 생활 습관을 짧게 기록해 보세요. 기록을 바탕으로 필요한 생활 관리와 진료 여부를 더 차분히 확인할 수 있습니다. 이 글에 제휴하면 좋은 제품 후보: **${product}**. 이는 사용자가 제휴 여부를 직접 판단할 참고 후보이며, 생활 변화를 기록하는 보조 도구로 진단·치료나 의료진의 안내를 대신하지는 않습니다.`;
}
