...
function buildPrompt(topic, refText, frame) {
  ...
  const manuallySelectedAffiliate = topic?.affiliateSelectionMode === 'manual';
...
- 최종 포스팅 제목은 “${selectedTitle}”입니다. 이 제목은 뉴스 기사 제목을 그대로 복사하지 않고 글감 단계에서 새로 만든 제목입니다. 단어·띄어쓰기·문장부호·어미를 바꾸거나 검색 키워드를 덧붙이지 말고 글자 하나까지 그대로 반환하세요.
  - ${selectedAffiliateProduct ? `실제 쇼핑커넥트 상품 “${selectedAffiliateProduct}”과 발급된 제휴 링크가 준비된 글입니다. CTA 아래 상품 카드는 발행 단계에서 자동으로 넣으므로 원고에 원시 URL이나 대가성 고지 문구를 직접 쓰지 마세요. 11 스킬의 “실제 상품과 링크가 있는 글” 흐름을 따르되, 상세정보로 확인하지 않은 소재·함량·기능·효과는 만들지 말고 일반적인 비교 기준만 안내하세요.` : `실제 상품 링크가 아직 없는 건강정보 글입니다. 마지막에는 독자가 글 내용에 맞는 행동을 바로 고를 수 있도록 생활형 CTA와 비처방 제품 후보 1개만 안내하세요.`}
${selectedAffiliateProduct ? `- ${manuallySelectedAffiliate ? '사용자가 입력한 검색어로 찾은' : '글 주제에 맞춰 자동으로 찾은'} 실제 제휴상품은 “${selectedAffiliateProduct}”입니다. 선정 방식을 독자에게 말하거나 글 앞부분부터 상품명을 반복하지 마세요. 먼저 증상·원인·생활관리와 비용 없이 해볼 방법을 충분히 설명한 뒤에만 정확한 상품명을 사용하세요. 비타민·영양제·건강식품이라면 구매 기준부터 꺼내지 말고 “이럴 때 식사에서 함께 챙겨볼 영양소로 비타민B군을 살펴볼 수 있습니다”처럼 현재 생활 문제와 영양소를 잇는 문장을 먼저 쓰세요. 이어 관련 영양소가 들어 있는 음식과 실제 식사 방법을 설명하고, 식사가 불규칙하거나 매번 준비하기 어려운 상황에서만 영양제품을 보완 선택지로 소개하세요. ‘영양성분 표시와 1회 섭취량을 확인하세요’ 같은 선택 기준 문장이 상품의 첫 소개 문장이 되면 안 됩니다. 생활도구라면 제품 없이 해볼 방법과 반복되는 준비·세척·휴대·보관의 번거로움을 먼저 짚은 뒤 연결하세요. 확인되지 않은 소재·함량·기능·효과·후기는 만들지 말고 제품 종류에 맞는 일반적인 구매 전 확인 기준만 안내하세요. 글의 마지막은 독자가 자신의 식사 또는 사용 환경을 확인하고 “${selectedAffiliateProduct}”이 자신에게 맞는 선택지인지 자연스럽게 살펴보도록 마무리하세요.` : '- 실제 제휴상품이 전달되지 않았습니다. 글의 핵심 생활 행동을 실제로 돕는 비처방 식품 또는 생활용품 후보 1개만 주제에 맞게 고르세요.'}
...
3-2. 본문 첫 번째 image 블록(slot 1)은 대표이미지 바로 아래에 두지 말고, 서론과 **이 글에서 확인할 핵심 N가지** 번호 목록이 모두 끝난 직후에 두세요. 첫 질문형 소제목보다 앞에 배치하세요.
...
【본문의 고정 흐름 — 순서를 바꾸지 마세요】
1. **문제 제기**: 서론에서 검색자가 겪는 구체적인 불편을 보여 주고 핵심 질문에 직접 답하세요.
2. **증상과 확인 기준**: 첫 본문 구간에서 어떤 증상·양상·시간·빈도·동반 변화가 나타날 수 있는지 설명해 독자가 자기 상태와 비교할 수 있게 하세요.
...
12. 행동 설명을 마친 뒤 별도의 마지막 paragraph 블록을 만드세요. ${selectedAffiliateProduct ? `앞에서 설명한 생활 문제와 선택 기준을 이어받아 “${selectedAffiliateProduct}”이 어떤 번거로움을 덜어 주는 보조 선택지인지 자연스럽게 설명하세요. 확인되지 않은 기능은 만들지 말고, 내 사용 환경에 맞는 크기·구조·관리 방법 또는 실제 표시사항을 판매 페이지에서 비교한 뒤 구매를 결정하도록 2~3문장으로 마무리하세요.` : '글 주제와 직접 맞는 비처방 제품 카테고리 1개를 골라, 어떤 생활 불편을 덜어 주는지와 실제로 비교할 기준을 2~3문장으로 자연스럽게 설명하세요.'} '제품 후보:', '이럴 때 살펴볼 제품:'처럼 제품명을 표지처럼 따로 적는 문장은 절대 만들지 마세요. 의료 수치나 법적 판단에 쓰이는 측정 도구라면 필요한 주의 문구를 CTA 안에 자연스럽게 설명하세요. 음료·조리 온도계 같은 생활용 온도 확인 도구에는 불필요한 진단 면책 문구를 붙이지 마세요. 해시태그 6개는 이 자연스러운 CTA 다음, 글 전체의 맨 마지막에만 배치합니다.
...
다음 JSON 형식으로만 출력:
{
  ...
  "blocks": [
    ...
    {"type": "image", "slot": 1, "caption": "건강 주제 첫 본문 이미지", "desc": "핵심 3가지 요약과 직접 연결되는 생활 장면"},
    ...
    {"type": "paragraph", "text": ${JSON.stringify(selectedAffiliateProduct ? `앞에서 설명한 불편이 반복된다면 **${selectedAffiliateProduct}**이 실천을 돕는 선택지인지 살펴볼 수 있습니다. 내 사용 환경에 맞는 조건과 실제 구성을 판매 페이지에서 비교한 뒤 필요한 경우 구매를 결정하세요.` : '앞의 생활 행동을 반복하기 어렵다면 글 주제와 직접 연결되는 비처방 제품이 그 번거로움을 덜어 주는지 살펴볼 수 있습니다. 실제 사용 환경과 관리 조건을 비교한 뒤 필요한 경우 구매를 결정하세요.')}}
  ]
}
...
const NEWS_AFFILIATE_RE = /(?:이\s*글에\s*제휴하면\s*좋은\s*제품(?:\s*후보)?|제품\s*후보|지정\s*제휴상품|이럴\s*때\s*살펴볼\s*제품)\s*[:：]/;
...
// 파이프라인에서 실제 연결 상품을 전달하면 그 상품을 사용하고,
// 단독 원고 생성처럼 상품이 없는 경우에만 안전한 후보를 제시한다.
function getSafeProductCandidate(topic = {}) {
  const selected = String(topic.affiliateProduct || topic.primaryProduct || '').trim();
  ...
  if (/혈변|치질|항문|선홍색\s*피|검붉은\s*변/.test(subject)) return '변기 설치형 좌욕기';
...
const HEALTH_KEY_SUMMARY_RE = /이 글에서 확인할 핵심\s*\d*가지/;
const HEALTH_NUMBERED_SUMMARY_ITEM_RE = /^\s*\d+\s*[.)]\s*/;

// 대표이미지는 퍼블리셔가 별도로 글 맨 위에 넣는다. 원고의 slot 1은 본문 첫
// 이미지이므로 서론과 핵심 요약 번호 목록이 끝난 뒤, 첫 소제목 앞에 둔다.
function placeFirstHealthImageAfterSummary(article) {
  const blocks = article.blocks || [];
  const firstImageIndex = blocks.findIndex(
    (block) => block.type === 'image' && Number(block.slot) === 1
  );
  const fallbackImageIndex = firstImageIndex >= 0
    ? firstImageIndex
    : blocks.findIndex((block) => block.type === 'image');
  if (fallbackImageIndex < 0) return article;

  const [firstBodyImage] = blocks.splice(fallbackImageIndex, 1);
  const summaryIndex = blocks.findIndex(
    (block) =>
      (block.type === 'paragraph' || block.type === 'heading') &&
      HEALTH_KEY_SUMMARY_RE.test(String(block.text || ''))
  );
  if (summaryIndex < 0) {
    blocks.splice(fallbackImageIndex, 0, firstBodyImage);
    article.blocks = blocks;
    return article;
  }

  let insertAt = summaryIndex + 1;
  const summaryText = String(blocks[summaryIndex].text || '');
  if (!/\n\s*\d+\s*[.)]\s*/.test(summaryText)) {
    while (
      insertAt < blocks.length &&
      blocks[insertAt].type === 'paragraph' &&
      HEALTH_NUMBERED_SUMMARY_ITEM_RE.test(String(blocks[insertAt].text || ''))
    ) {
      insertAt += 1;
    }
  }
  blocks.splice(insertAt, 0, firstBodyImage);
  article.blocks = blocks;
  return article;
}
...
function makeRecommendationBlocks(topic) {
  ...
  return [
    {
      type: 'paragraph',
      text: selectedProductPurchaseCta(product),
    },
  ];
}
...
function productCandidateFrom(text, topic) {
  const selected = String(topic?.affiliateProduct || topic?.primaryProduct || '').trim();
  if (selected) return selected;
  const match = String(text || '').match(/(?:이\s*글에\s*제휴하면\s*좋은\s*제품(?:\s*후보)?|제품\s*후보|지정\s*제휴상품|이럴\s*때\s*살펴볼\s*제품)\s*[:：]\s*\*\*([^*]+)\*\*/);
  const product = String(match?.[1] || getSafeProductCandidate(topic)).trim();
  if (/(?:기록|건강|증상|배변)\s*(?:노트|수첩|일지|다이어리)|기록용\s*(?:노트|수첩|일지)/.test(product)) {
    return getSafeProductCandidate(topic);
  }
  return product;
}
...
function isNutritionProduct(product) {
  return /비타민|영양제|건강기능식품|보충제|프로바이오틱스|유산균|오메가|미네랄|마그네슘|철분|아연|칼슘|요거트|식품|음료|차\b/.test(String(product || ''));
}

function nutritionFocusFromProduct(product) {
  const value = String(product || '');
  if (/비타민\s*[bB비](?:군|\s*군|\d*)/.test(value)) return '비타민B군';
  if (/비타민\s*[cC씨]/.test(value)) return '비타민C';
  if (/비타민\s*[dD디]/.test(value)) return '비타민D';
  if (/마그네슘/.test(value)) return '마그네슘';
  if (/철분|철\b/.test(value)) return '철분';
  if (/아연/.test(value)) return '아연';
  if (/칼슘/.test(value)) return '칼슘';
  if (/오메가\s*-?\s*3/.test(value)) return '오메가3 지방산';
  if (/프로바이오틱스|유산균/.test(value)) return '프로바이오틱스';
  return '관련 영양소';
}

function selectedProductPurchaseCta(product) {
  if (isNutritionProduct(product)) {
    const nutrient = nutritionFocusFromProduct(product);
    return `이럴 때 식사에서 함께 챙겨볼 영양소로 **${nutrient}**을 살펴볼 수 있습니다. 먼저 ${nutrient}이 들어 있는 음식을 평소 식사에 고르게 더하고, 식사가 불규칙해 매번 챙기기 어렵다면 보완 선택지로 **${product}**도 비교해 보세요. 실제 원재료와 1회 섭취량, 알레르기 표시, 보관 방법을 확인한 뒤 내 식사에 필요한 경우 구매를 결정하면 됩니다.`;
  }
  if (/방석|쿠션/.test(String(product || ''))) {
    return `오래 앉아 있을 때 좌판이 딱딱하거나 자세가 자꾸 흐트러진다면 **${product}**을 앉는 환경을 편하게 조정하는 보조 도구로 비교해 볼 수 있습니다. 의자 좌판에 맞는 크기, 몸이 한쪽으로 과하게 기울지 않는 구조, 앉고 일어나기 편한 높이와 관리 방법을 확인한 뒤 내 생활에 맞는 제품인지 판매 페이지에서 살펴보세요.`;
  }
  return `앞에서 확인한 사용 환경과 선택 기준이 내 생활에 맞는다면, **${product}**의 실제 구성·사용 방법·관리 조건을 판매 페이지에서 확인해 보세요. 필요한 조건이 맞는지 다른 선택지와 비교한 뒤 구매를 결정하면 됩니다.`;
}

const SELECTED_PRODUCT_PURCHASE_CTA_RE = /판매\s*페이지|구매를\s*(?:결정|검토)|실제\s*(?:구성|상품\s*정보).{0,30}(?:확인|비교)|함께\s*챙겨볼\s*영양소|식사를\s*보완하는\s*선택지/;

// 예전 원고의 제품 표지 문구를 제거하고, 제품의 쓰임과 비교 기준을
// 자연스럽게 설명하는 CTA 문단 하나로 마무리한다.
function ensureHealthRecommendation(article, topic) {
  const blocks = article.blocks || [];
  let product = getSafeProductCandidate(topic);

  for (let index = blocks.length - 1; index >= 0; index -= 1) {
    const block = blocks[index];
    if (block.type !== 'paragraph' || !NEWS_AFFILIATE_RE.test(block.text || '')) continue;
    const originalText = String(block.text || '');
    product = productCandidateFrom(originalText, topic);
    const markerIndex = originalText.search(NEWS_AFFILIATE_RE);
    const actionText = originalText
      .slice(0, markerIndex)
      .replace(/이런\s*실천을\s*더\s*편하게\s*이어\s*가는\s*제품도\s*있습니다\.?/g, '')
      .replace(/\s+/g, ' ')
      .trim();
    if (actionText) blocks.splice(index, 1, { ...block, text: actionText });
    else blocks.splice(index, 1);
  }

  if (NEWS_MEDICINE_RE.test(product)) product = getSafeProductCandidate({ ...topic, affiliateProduct: '', primaryProduct: '' });

  const hasNaturalCta = blocks.some(
    (block) =>
      block.type === 'paragraph' &&
      String(block.text || '').includes(product) &&
      SELECTED_PRODUCT_PURCHASE_CTA_RE.test(block.text || '')
  );
  if (!hasNaturalCta) blocks.push({ type: 'paragraph', text: selectedProductPurchaseCta(product) });
...
function inspectNewsArticle(article) {
  ...
  const summaryBlocks = (article.blocks || []).filter(
    (block) =>
      (block.type === 'paragraph' || block.type === 'heading') &&
  ...
  const affiliateIndex = paragraphs.findIndex(
    (block) => NEWS_AFFILIATE_RE.test(block.text || '') || SELECTED_PRODUCT_PURCHASE_CTA_RE.test(block.text || '')
  );
  const affiliateParagraph = affiliateIndex >= 0 ? paragraphs[affiliateIndex]?.text || '' : '';
  const actionParagraph = affiliateIndex > 0
    ? `${paragraphs[affiliateIndex - 1]?.text || ''} ${affiliateParagraph}`
    : affiliateParagraph || paragraphs.at(-1)?.text || '';
  if (!NEWS_CTA_ACTION_RE.test(actionParagraph)) issues.push('마지막 생활형 CTA에 구체적인 행동 제안 없음');
  if (!affiliateParagraph) issues.push('제품의 쓰임과 비교 기준을 설명하는 마지막 CTA 없음');
...
async function writeArticle(topic, refs) {
  ...
  article = normalizeHealthProductFlow(
    ensureHealthRecommendation(
      placeFirstHealthImageAfterSummary(
...
- 서론에는 "이 글에서 확인할 핵심 N가지"를 정확히 한 번만 넣고, 각 항목을 이 글의 구체적인 원인·확인 기준·실천법으로 작성하세요. 어느 건강 글에도 붙일 수 있는 범용 문구는 쓰지 마세요. 제목이 숫자를 약속했다면 같은 숫자로 맞추세요.
