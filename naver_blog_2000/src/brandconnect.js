...
const CREATOR_ID = '981491868759168'; // 현재 로그인 계정의 브랜드커넥트 크리에이터 ID
...
// 브랜드커넥트는 숨김 브라우저에서 오류 페이지를 반환하므로 실제 브라우저 창을 쓴다.
async function withContext(fn) {
  const browser = await browserHelper.launch({ headless: false });
  try {
...
async function dismissBrandConnectPrompts(page) {
  const laterButton = page.locator('button:has-text("다음에 입력하기")').first();
  if (await laterButton.isVisible().catch(() => false)) {
    await laterButton.click().catch(() => {});
    await sleep(500);
  }
  const closeButton = page.locator('button[aria-label="팝업 닫기"]').first();
...
async function searchInContext(context, query, { maxCount = 8 } = {}) {
  ...
  await sleep(6500);
  await dismissBrandConnectPrompts(page);
...
async function suggestHealthProductKeywords(topic, refs = []) {
  const referenceText = refs
    ...
    .join('\n\n')
    .slice(0, 6000);
  const requestedProductQuery = String(topic?.requestedProductQuery || '').trim();
  const prompt = `아래 건강 기사와 글감에서 억지스럽지 않게 연결할 수 있는 주력 상품과 관련 대체 상품군을 정하세요.

글감: ${topic?.title || ''}
관점: ${topic?.angle || ''}
${requestedProductQuery ? `사용자가 원하는 1순위 검색어: ${requestedProductQuery}` : ''}
참고자료:
${referenceText}
...
- 한 글에는 실제 상품 카드 하나만 연결합니다.
- 사용자가 1순위 검색어를 입력했다면 primaryProduct와 keywords 첫 항목에 그 검색어를 그대로 유지합니다.
- keywords는 쇼핑커넥트에서 실제로 검색할 수 있는 짧은 상품군 2~3개입니다.
- 첫 항목은 1순위 상품군, 다음 항목은 1순위 결과가 없을 때 사용할 관련 2순위 상품군입니다.
- 2순위 상품군도 글의 핵심 생활 행동을 직접 돕고 치료·예방 효과를 주장하지 않는 제품이어야 합니다.

JSON 형식:
{"primaryProduct":"1순위 상품군","productReason":"필요한 현실적인 이유","keywords":["1순위 상품 검색어","관련 2순위 상품 검색어","관련 3순위 상품 검색어"]}`;
