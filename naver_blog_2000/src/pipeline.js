...
async function publishAndNotify(draftId, article, judgments, visibility, mode = 'draft', publishOptions = {}) {
  ...
  // 참고자료는 사실 확인용 내부 기록으로만 보관하고 네이버 본문에는 넣지 않는다.
...
async function placeHealthImagesAndSave(draftId, visibility = 'public', finalMode = 'draft') {
  ...
  // 이전에 저장된 원고라도 최신 배치 규칙을 적용해 미리보기와 네이버 본문 순서를 맞춘다.
  writer.placeFirstHealthImageAfterSummary(article);
  ...
  try {
    const result = await publishAndNotify(
      ...
      finalMode,
      {
        ...
        returnResult: true,
        doneLabel: `${finalMode === 'publish' ? '네이버 발행' : '2차 임시저장'} 완료 · 대표이미지 1장 + 본문 이미지 ${bodyImageCount}장 배치`,
        donePatch: {
...
async function completeHealthImagesAndPlacement(draftId, visibility = 'public', finalMode = 'draft') {
  ...
  const placement = await placeHealthImagesAndSave(
    draftId,
    visibility || meta.visibility || 'public',
    finalMode
  );
...
async function resumePendingHealthDrafts() {
  ...
  for (const draft of pending) {
    try {
      if (draft.imagesPending) {
        await completeHealthImagesAndPlacement(
          draft.id,
          draft.visibility || 'public',
          draft.mode || 'draft'
        );
      } else {
        await placeHealthImagesAndSave(
          draft.id,
          draft.visibility || 'public',
          draft.mode || 'draft'
        );
...
async function prepareHealthAffiliateProduct(requestedQuery, topic, refs, setStep) {
  let productPlan = {
    primaryProduct: requestedQuery,
    productReason: requestedQuery
      ? '사용자가 입력한 검색어와 글의 생활관리 흐름을 자연스럽게 연결'
      : '',
    keywords: requestedQuery ? [requestedQuery] : [],
  };

  setStep('collecting', requestedQuery
    ? '입력한 상품과 관련된 대체 상품군 정하는 중'
    : '글 주제에 맞는 제휴상품 검색어 정하는 중');
  try {
    productPlan = await brandconnect.suggestHealthProductKeywords(
      { ...topic, requestedProductQuery: requestedQuery || undefined },
      refs
    );
  } catch (error) {
    if (!requestedQuery) throw error;
    console.log(`[pipeline] 관련 대체 상품군 생성 실패(입력 검색어만 사용): ${error.message}`);
  }

  const keywords = [
    requestedQuery,
    productPlan.primaryProduct,
    ...(Array.isArray(productPlan.keywords) ? productPlan.keywords : []),
  ]
    .map((value) => String(value || '').replace(/\s+/g, ' ').trim())
    .filter((value, index, values) => value && values.indexOf(value) === index)
    .slice(0, 5);
  if (!keywords.length) {
    throw new Error('글 주제에 맞는 제휴상품 검색어를 만들지 못했습니다.');
  }
  ...
  for (let index = 0; index < keywords.length; index += 1) {
    ...
    setStep(
      'collecting',
      index === 0
        ? `쇼핑커넥트에서 1순위 상품 찾는 중 (${keyword})`
        : `관련 ${index + 1}순위 상품군에서 찾는 중 (${keyword})`
    );
    const candidate = await brandconnect.getBestProduct({ keywords: [keyword] });
    if (!candidate || !candidate.name || !candidate.url) {
      console.log(`[pipeline] 쇼핑커넥트 검색 결과 없음 (${keyword}) — 다음 상품군 확인`);
      continue;
    }

    setStep('collecting', `제휴 링크 발급 중 (${candidate.name.slice(0, 36)})`);
    const issuedLink = await brandconnect.issueAffiliateLink(candidate.url).catch((error) => {
      console.log(`[pipeline] 제휴 링크 발급 실패 (${keyword}): ${error.message}`);
      return '';
    });
    if (!/^https?:\/\//i.test(String(issuedLink || ''))) continue;
  ...
  if (!product || !link) {
    throw new Error(`쇼핑커넥트 1·2순위 상품군에서 연결할 상품을 찾지 못했습니다: ${keywords.join(', ')}`);
  }
  ...
  return {
    ...
    keywords,
    selectedKeyword,
    fallbackUsed: selectedKeyword !== keywords[0],
    productReason: String(productPlan.productReason || '').trim(),
  };
}
...
async function run(draftId, search, topic, visibility, opts = {}) {
  ...
  try {
    ...
    const affiliate = await prepareHealthAffiliateProduct(
      affiliateProduct,
      { ...topic, angle: withoutAffiliateAngle(topic.angle) },
      refs,
      setStep
    );
    const linkedProduct = {
      name: affiliate.product.name,
      price: affiliate.product.price,
      commission: affiliate.product.commission,
      image: affiliate.product.image || '',
      link: affiliate.link,
      searchKeyword: affiliate.selectedKeyword,
      fallbackUsed: affiliate.fallbackUsed,
    };
    const healthTopic = {
      ...topic,
      angle: withoutAffiliateAngle(topic.angle),
      affiliateProduct: affiliate.product.name,
      primaryProduct: affiliate.product.name,
      affiliateSelectionMode: affiliateProduct ? 'manual' : 'auto',
      productReason: affiliate.productReason || undefined,
      productKeywords: affiliate.keywords,
    };

    // 2. 실제 발급된 제휴상품을 저장해 원고 CTA와 네이버 상품 카드가 같은 상품을 사용하게 한다.
    store.updateDraft(draftId, {
      ...
      healthPlan: {
        ...
        affiliateProduct: affiliate.product.name,
        productReason: affiliate.productReason || '',
        productKeywords: affiliate.keywords,
        selectedProductKeyword: affiliate.selectedKeyword,
        fallbackUsed: affiliate.fallbackUsed,
      },
    ...
    const postUrl = await publishAndNotify(draftId, article, judgments, visibility, 'draft', {
      intermediate: true,
      nextStatus: 'images',
