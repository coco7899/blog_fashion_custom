...
const SEL = {
  ...
  shoppingConnectBtn: '.se-shopping-connect-toolbar-button',
  shoppingConnectItemName: '.se-shopping-connect-item-name',
  shoppingConnectItemAdd: 'button.se-shopping-connect-item-add-button',
  shoppingConnectConfirm: '.se-popup-button-confirm',
  shoppingConnectClose: '.se-popup-close-button',
  imageAiMark: '.se-set-ai-mark-button',
  imageAiToggle: '.se-set-ai-mark-button-toggle',
...
function cleanProductPostArticle(article, products) {
  ...
  // 쇼핑커넥트 카드를 에디터에서 추가하면 네이버가 대가성 고지 문구를
  // 글 상단에 자동 삽입한다. 여기서 직접 넣으면 같은 고지가 두 번 생긴다.
...
function shoppingConnectTokens(value) {
  return String(value || '')
    .toLowerCase()
    .replace(/[^0-9a-z가-힣]+/g, ' ')
    .split(/\s+/)
...
async function findBestShoppingConnectItem(frame, productName, searchKeyword = '') {
  const items = frame.locator(SEL.shoppingConnectItemName);
  const count = await items.count().catch(() => 0);
  const productTokens = shoppingConnectTokens(productName);
  const keywordTokens = shoppingConnectTokens(searchKeyword);
  let best = null;
  let bestScore = -1;
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const itemName = String(await item.innerText().catch(() => '')).replace(/\s+/g, ' ').trim();
    if (!itemName) continue;
    const normalizedItem = itemName.toLowerCase();
    const normalizedProduct = productName.toLowerCase();
    const itemTokens = new Set(shoppingConnectTokens(itemName));
    let score = 0;
    if (normalizedItem === normalizedProduct) score += 200;
    else if (normalizedItem.includes(normalizedProduct) || normalizedProduct.includes(normalizedItem)) score += 100;
    score += productTokens.filter((token) => itemTokens.has(token)).length * 4;
    score += keywordTokens.filter((token) => itemTokens.has(token)).length * 10;
    if (keywordTokens.length && keywordTokens.every((token) => itemTokens.has(token))) score += 40;
    if (score > bestScore) {
      best = { item, itemName };
      bestScore = score;
    }
...
async function insertShoppingConnectProduct(frame, page, product) {
  const productName = String(product?.name || '').replace(/\s+/g, ' ').trim();
  const searchKeyword = String(product?.searchKeyword || '').replace(/\s+/g, ' ').trim();
  if (!productName) throw new Error('쇼핑커넥트에 추가할 상품명이 없습니다.');

  const opened = await clickIfVisible(frame, SEL.shoppingConnectBtn, 5000);
  if (!opened) throw new Error('네이버 에디터의 쇼핑커넥트 버튼을 찾지 못했습니다.');
  await sleep(1800);

  let matched = await findBestShoppingConnectItem(frame, productName, searchKeyword);
  if (!matched) {
    const searchInput = frame.locator('input[placeholder*="쇼핑 커넥트 상품"]').first();
    if (!(await searchInput.isVisible().catch(() => false))) {
      throw new Error(`발급된 쇼핑커넥트 상품을 찾지 못했습니다: ${productName}`);
    }
    const searchTerms = [searchKeyword, productName.slice(0, 36)]
    ...
    for (const term of searchTerms) {
      ...
      await page.keyboard.press('Enter');
      await sleep(1800);
      matched = await findBestShoppingConnectItem(frame, productName, searchKeyword || term);
      if (matched) break;
    }
  }
  if (!matched || !(await matched.item.isVisible().catch(() => false))) {
    throw new Error(`쇼핑커넥트 상품 선택창에 상품이 없습니다: ${productName}`);
  }

  const itemRoot = matched.item.locator(
    'xpath=ancestor::*[.//button[contains(@class,"se-shopping-connect-item-add-button")]][1]'
  );
  await itemRoot.locator(SEL.shoppingConnectItemAdd).click({ timeout: 5000 });
  await sleep(500);

  const confirm = frame.locator(SEL.shoppingConnectConfirm).filter({ hasText: '추가하기' }).first();
  if (!(await confirm.isVisible().catch(() => false))) {
    throw new Error('쇼핑커넥트 상품 추가 확인창을 찾지 못했습니다.');
  }
  await confirm.click({ timeout: 5000 });
  await sleep(1500);
  await clickIfVisible(frame, SEL.shoppingConnectClose, 1200);
  await sleep(300);

  const insertedCount = await frame
    .locator('.se-component')
    .filter({ hasText: matched.itemName })
    .count()
    .catch(() => 0);
  if (!insertedCount) throw new Error(`쇼핑커넥트 상품 카드 삽입을 확인하지 못했습니다: ${matched.itemName}`);
  return matched.itemName;
}

async function focusBelowShoppingConnectProduct(frame, page, productName) {
  const focusedBottom = await clickIfVisible(frame, '.se-canvas-bottom-button', 1500);
  if (focusedBottom) {
    await sleep(400);
    return;
  }

  // 상품 카드가 마지막 구성요소이면 하단 추가 버튼이 보이지 않을 수 있다.
  // 이때 카드를 선택 해제한 뒤 아래 방향키로 카드 다음 일반 문단으로 이동한다.
  const productCard = frame
    .locator('.se-component.se-shoppingConnect')
    .filter({ hasText: productName })
    .last();
  if (!(await productCard.isVisible().catch(() => false))) {
    throw new Error('쇼핑커넥트 상품 카드 아래 입력 위치를 찾지 못했습니다.');
  }
  await productCard.click({ timeout: 4000 });
...
async function verifyProductCardBeforeTags(frame, productName, tagLine) {
  ...
  const productIndex = order.findIndex(
    (item) => item.className.includes('se-shoppingConnect') && item.text.includes(productName)
  );
  const firstTag = String(tagLine || '').split(/\s+/).find(Boolean) || '';
  const tagIndex = order.findIndex((item) => firstTag && item.text.includes(firstTag));
  if (productIndex < 0) throw new Error('쇼핑커넥트 상품 카드 순서를 확인하지 못했습니다.');
  if (tagIndex < 0) throw new Error('상품 카드 아래 해시태그를 확인하지 못했습니다.');
  if (productIndex >= tagIndex) {
    throw new Error('해시태그가 쇼핑커넥트 상품 카드 아래에 배치되지 않았습니다.');
  }
}

async function verifyConclusionBeforeProductCard(frame, conclusionText, productName) {
  const order = await frame
    .locator('.se-component:not(.se-documentTitle)')
    .evaluateAll((nodes) =>
      nodes.map((node) => ({
  ...
  const productIndex = order.findIndex(
    (item) => item.className.includes('se-shoppingConnect') && normalizeEditorText(item.text).includes(normalizeEditorText(productName))
  );
  if (productIndex < 0) throw new Error('결론 문단 뒤의 쇼핑커넥트 상품 카드를 확인하지 못했습니다.');

  const expected = normalizeEditorText(String(conclusionText || '').replace(/\*\*/g, '')).replace(/\s/g, '');
  const beforeProduct = normalizeEditorText(
    order.slice(0, productIndex).map((item) => item.text).join(' ')
  ).replace(/\s/g, '');
  if (!expected || !beforeProduct.includes(expected)) {
    throw new Error('미리보기의 결론 문단 전체가 네이버 본문에 입력되지 않아 저장·발행을 중단했습니다.');
  }
}
...
async function verifySavedDraftByTitle(frame, page, title, expectedImageCount) {
  ...
  await verifyHealthImagePresentation(frame, expectedImageCount);
...
async function ensureAiUsageEnabled(image) {
  await image.click({ timeout: 4000 }).catch(() => {});
  const mark = image.locator(SEL.imageAiMark).first();
  await mark.waitFor({ state: 'visible', timeout: 5000 }).catch(() => {});
  if (!(await mark.count().catch(() => 0))) {
    throw new Error('이미지의 AI 활용 설정 버튼을 찾지 못했습니다.');
  }

  const toggle = mark.locator(SEL.imageAiToggle).first();
  let enabled = await toggle.evaluate((node) => node.classList.contains('se-is-selected')).catch(() => false);
  if (!enabled) {
    await toggle.click({ timeout: 4000 });
    await sleep(350);
    enabled = await toggle.evaluate((node) => node.classList.contains('se-is-selected')).catch(() => false);
  }
  if (!enabled) throw new Error('이미지의 AI 활용 설정을 켜지 못했습니다.');
}

async function verifyHealthImagePresentation(frame, expectedImageCount) {
  const images = frame.locator('.se-component.se-image');
  const actualImageCount = await images.count().catch(() => 0);
  if (actualImageCount !== expectedImageCount) {
    throw new Error(`이미지 표시 규칙 확인 중 ${actualImageCount}장만 보입니다. 기대 ${expectedImageCount}장`);
  }

  const enabledAiCount = await frame
    .locator('.se-component.se-image .se-set-ai-mark-button-toggle.se-is-selected')
    .count()
    .catch(() => 0);
  if (enabledAiCount !== expectedImageCount) {
    throw new Error(`AI 활용 설정이 ${enabledAiCount}/${expectedImageCount}장만 켜져 있습니다.`);
  }

  const imageCaptionTexts = await frame
    .locator('.se-component.se-image .se-caption .__se-node')
    .evaluateAll((nodes) => nodes.map((node) => (node.textContent || '').trim()).filter(Boolean))
    .catch(() => []);
  if (imageCaptionTexts.length) throw new Error('이미지 아래 사진 설명이 남아 있습니다.');

  const componentOrder = await frame
    .locator('.se-component:not(.se-documentTitle)')
    .evaluateAll((nodes) => nodes.map((node) => ({
      image: node.classList.contains('se-image'),
      text: node.innerText || '',
    })))
    .catch(() => []);
  const summaryIndex = componentOrder.findIndex((item) => /이 글에서 확인할 핵심\s*\d*가지/.test(item.text));
  const imageIndexes = componentOrder
    .map((item, index) => (item.image ? index : -1))
    .filter((index) => index >= 0);
  if (summaryIndex >= 0 && imageIndexes.length >= 2 && imageIndexes[1] <= summaryIndex) {
    throw new Error('본문 첫 이미지가 핵심 요약보다 앞에 배치되어 있습니다.');
  }
}

async function insertImage(frame, page, filePath, { alignLeft = false, enableAiUsage = false } = {}) {
  ...
  if (enableAiUsage) await ensureAiUsageEnabled(latestImage);
  if (alignLeft) {
    // 쇼핑 포스팅은 이미지 블록까지 왼쪽에 맞춘다.
    await latestImage.click({ timeout: 2500 }).catch(() => {});
    await setAlign(frame, page, 'left');
  }
  // 이미지 선택 상태만 해제하고 다음 본문 입력 위치로 이동한다.
  // 사진 설명은 네이버 에디터에 쓰지 않고 내부 장면 정보와 파일명으로만 보관한다.
  await page.keyboard.press('Escape');
  await sleep(300);
  await page.keyboard.press('ArrowDown');
  await sleep(200);
...
async function publish(article, judgments, opts) {
  ...
  // 마지막 제품 CTA 뒤에 쇼핑커넥트 카드를 넣고, 해시태그는 카드 아래 글 전체 맨 끝에 둔다.
  ...
  const deferredCtaText = String(deferredCtaBlock?.text || '')
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean)
    .join(' ');
  ...
  try {
    ...
    for (const [blockIndex, block] of publishBlocks.entries()) {
      ...
      if (block.type === 'heading') {
        ...
        if (j && j.file) {
          ...
          try {
            await insertImage(frame, page, path.join(imagesDir, 'raw', j.file), {
              alignLeft: hasProducts,
              enableAiUsage: !hasProducts || Boolean(j.ai || j.generated),
            });
          } catch (e) {
    ...
    // 건강 참고자료는 내부 검증용으로만 보관하고 네이버 본문에는 넣지 않는다.

    // 건강 글은 앞 문단 입력 뒤 이미 Enter를 두 번 눌렀으므로 자연스러운 CTA를
    // 바로 이어 쓴다. 실제 상품 링크가 있는 글만 기존 간격을 추가한다.
    if (deferredCtaText) {
      if (hasProducts) {
        await page.keyboard.press('Enter');
        await page.keyboard.press('Enter');
        // 상품 카드 직전 결론은 줄 단위 Enter로 나누면 일부 문장이 선택 상태에
        // 남아 카드 삽입 때 유실될 수 있으므로 하나의 문단으로 입력한다.
        await typeRich(page, deferredCtaText);
      } else {
        await typeParagraph(page, deferredCtaText);
      }
      await page.keyboard.press('Enter');
      await page.keyboard.press('Enter');
    }

    // ── 주력 상품 쇼핑커넥트 카드 (CTA 바로 아래) ──────────
    const linkProducts = (products || []).filter((p) => p && p.link).slice(0, 5);
    let insertedProductName = '';
    if (linkProducts.length) {
      onStep('쇼핑커넥트 상품 카드 삽입 중');
      // 건강 글은 한 글에 주력 상품 하나만 연결한다.
      insertedProductName = await insertShoppingConnectProduct(frame, page, linkProducts[0]);

      await focusBelowShoppingConnectProduct(frame, page, insertedProductName);
    }

    // ── 해시태그 (상품 카드 아래, 글 전체 맨 끝) ───────────
    ...
    if (insertedProductName && tagLine) {
      await verifyProductCardBeforeTags(frame, insertedProductName, tagLine);
    }
    if (insertedProductName && deferredCtaText) {
      await verifyConclusionBeforeProductCard(frame, deferredCtaText, insertedProductName);
    }
    ...
    if (strictImages) {
      ...
      if (insertCover) await verifyHealthImagePresentation(frame, requiredImageCount);
    }
