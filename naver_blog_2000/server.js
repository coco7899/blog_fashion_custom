...
app.get('/api/status', (req, res) => {
  ...
  res.json({
    ...
    blogId: blogProfile.blogId || null,
    blogUrl: blogProfile.blogId ? `https://blog.naver.com/${blogProfile.blogId}` : null,
...
app.post('/api/schedule/control', async (req, res) => {
  ...
  if (command === 'start') {
    const login = await auth.verify();
    if (!login.loggedIn) {
      return res.status(401).json({ error: '네이버 로그인이 필요합니다.' });
    }
  }
  try {
...
app.post('/api/drafts/:id/retry-images', (req, res) => {
  ...
  const work = meta.imagePlacementRequired
    ? pipeline.completeHealthImagesAndPlacement(
        id,
...
app.post('/api/drafts/:id/retry-image-placement', (req, res) => {
  ...
  pipeline.placeHealthImagesAndSave(
    id,
    meta.visibility || 'public',
    meta.mode || 'draft'
  ).catch((error) => {
