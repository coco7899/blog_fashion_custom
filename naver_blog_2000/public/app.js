...
async function refreshStatus() {
  try {
    ...
    if (targetBlogLink && s.blogUrl) {
      ...
      targetBlogLink.textContent = `발행 대상 · ${s.blogId || '네이버 블로그'}`;
    }
    if (s.session) {
...
$('linkModeBtn').onclick = async () => {
  ...
  const actLabel = mode === 'publish' ? `이미지까지 넣어 ${visibility === 'private' ? '비공개' : '공개'} 발행` : '네이버 임시저장';
...
async function startRun(topicIndex, visOverride) {
  ...
  const actLabel = mode === 'publish' ? `이미지까지 넣어 ${visibility === 'private' ? '비공개' : '공개'} 발행` : '네이버 블로그에 임시저장';
...
async function processQueue(visOverride) {
  ...
  try {
    ...
    if (/네이버.*로그인|로그인.*네이버/.test(e.message)) {
      runningTopicIndex = null;
      runQueue = [];
      queuedAffiliateProducts.clear();
      refreshTopicButtons();
  ...
  drafts.forEach((d) => {
    ...
    if (republishBtn) {
      republishBtn.onclick = async () => {
        if (!(await uiConfirm('작성된 원고와 이미지를 같은 네이버 글에 다시 공개 발행할까요?'))) return;
