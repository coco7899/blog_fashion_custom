...
assert.match(publisherSource, /saveTemporaryDraft/);
assert.match(publisherSource, /티스토리 임시저장 버튼을 찾지 못했습니다/);
assert.match(publisherSource, /작성 중인 글이 저장되었습니다/);
assert.match(publisherSource, /if \(mode === 'draft'\)/);
assert.match(pipelineSource, /representativeImageSet: Boolean\(result\.representativeImageSet\)/);
assert.match(pipelineSource, /generateHealthImagesAndPublish/);
assert.match(pipelineSource, /prePublishImageGeneration: true/);
...
assert.match(dashboardScript, /autoStartBtn/);
assert.match(dashboardScript, /autoStopBtn/);
assert.match(dashboardScript, /controlAutoPublish\('시작'\)/);
assert.match(dashboardScript, /controlAutoPublish\('중지'\)/);
assert.match(dashboardScript, /현재 글을 작성·발행하고 있습니다\. 이번 실행에서 완료한 글은 \$\{status\.sessionCount\}개입니다/);
assert.doesNotMatch(dashboardScript, /autoCommandInput|autoCommandBtn/);
assert.match(dashboardScript, /refreshAutoPublishStatus/);
assert.match(dashboardHtml, /<option value="draft">임시저장<\/option>/);
assert.doesNotMatch(dashboardHtml, /비공개 저장 \(직접 발행\)/);
assert.match(dashboardScript, /임시저장에서는 공개 설정을 사용하지 않습니다/);
